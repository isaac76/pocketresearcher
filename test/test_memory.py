...
class TestMemory(unittest.TestCase):
    ...
    def test_file_backend_load_and_save(self, mock_exists, mock_file):
        mem = Memory({"backend": MemoryBackend.FILE, "file_path": "test_memory.json"})
        loaded = mem.load()
    ...
    def test_mongodb_backend_load_and_save(self, mock_pymongo):
        ...
        mem = Memory({"backend": MemoryBackend.MONGODB, "mongo_uri": "mongodb://localhost", "mongo_db": "testdb", "mongo_collection": "memory"})
        loaded = mem.load()
        self.assertEqual(loaded["facts"], ["A fact"])
        mem.save(self.sample_memory)
